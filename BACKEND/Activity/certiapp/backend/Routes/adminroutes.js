    ...
    if (!result) {
      ...
      if (info) {
        const token = jwt.sign(
          { username: username, role: result.role },
          secretKey,
          { expiresIn: "1h" }
        ); // built in funtion jwt.sign
        res.cookie("authtoken", token, { httpOnly: true }); //not give access  to javascript
...
adminroute.post("/issuecertificate", authenticate, (req, res) => {
  try {
    const data = req.body;
    const { coursename, certificateid, Candidatename, SelectGrade, IssueDate } =
      data;
    const role = req.Role;
    if (role !== "admin") {
      ...
      if (certidetails.has(Candidatename)) {
        ...
        certidetails.set(Candidatename, {
          coursename,
          certificateid,
          Candidatename,
          SelectGrade,
        ...
        const detailcert =
          "This is to certify that ${Candidatename} has successfully completed the course ${coursename} with a grade of ${SelectGrade}.Certificate ID: ${certificateid} Issued on: ${IssueDate}";
